...
$DeployDir = "C:\Users\yomiy\Desktop\VillyCar_Webhost_Chile"
...
Write-Host "Creando carpeta API para el backend..."
New-Item -ItemType Directory -Force -Path "$DeployDir\api" | Out-Null

Write-Host "Copiando archivos del backend..."
Copy-Item -Path "src\backend\api\*" -Destination "$DeployDir\api" -Recurse -Force
...
$DbConfigFile = "$DeployDir\api\db.php"
...
$content = $content.Replace("`$user = 'root';", "`$user = 'joel';")
$content = $content.Replace("`$pass = '';", "`$pass = 'FSKrY2w`$hi-w#TN^';")
Set-Content -Path $DbConfigFile -Value $content -Encoding Ascii

Write-Host "=========================================="
Write-Host "¡Carpeta VillyCar_Webhost_Chile lista en tu Escritorio!"
Write-Host "=========================================="
